...
const AttestationSuccess: React.FC<AttestationSuccessProps> = ({
  ...
  const twitterShareUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(`I've just verified my content with DataGuardians! Check it out:`)}%0A${encodeURIComponent(verificationUrl)}`;
  
  const linkedinShareUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(verificationUrl)}&title=${encodeURIComponent('Content Verified with DataGuardians')}&summary=${encodeURIComponent('I\'ve verified my digital content authenticity with DataGuardians blockchain technology.')}`;
